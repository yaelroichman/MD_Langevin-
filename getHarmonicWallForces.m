function [fx,fy]=getHarmonicWallForces(particlesMat, R, K, wallPositionsX, wallPositionsY)
...
for currParticle = 1:numOfParticles
    ...
    if xBeyondRightWall > 0
        fx(currParticle) = fx(currParticle) - K*xBeyondRightWall;
    end
    if xBeyondLeftWall > 0
        fx(currParticle) = fx(currParticle) + K*xBeyondLeftWall;
    end
    if yBeyondTopWall > 0
        fy(currParticle) = fy(currParticle) - K*yBeyondTopWall;
    end
    if yBeyondBottomWall > 0
        fy(currParticle) = fy(currParticle) + K*yBeyondBottomWall;
    end
end